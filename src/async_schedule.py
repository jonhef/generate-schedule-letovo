...
class Schedule:
    ...
    async def get_schedule(self, first_day: datetime.datetime = None) -> "Week":
        ...
        res = Week([], today)
        for i in range(0, 7):
            req = await self.session.get(f"https://elk.letovo.ru/api/education_track/schedule?date={today.year}-{today.month}-{today.day}")
...
class StudentLetovo(Schedule):
    async def get_schedule(self, first_day: datetime.datetime = None) -> Week:
        res = await super().get_schedule(first_day)
    ...
    async def login(self, login: str = None, password: str = None) -> bool:
        ...
        req = await self._request_post("https://elk.letovo.ru/api/login", data=data, headers={"Content-Type": "application/json;charset=UTF-8"})
